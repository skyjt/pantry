...
- Linux job 使用 `node:18-buster` / Debian 10 容器，apt 指向 archive 源，并在 `npm ci` 后强制源码重建 better-sqlite3，输出 `Pantry-<version>-linux-x64.deb` 与 `Pantry-<version>-linux-x64.AppImage`，同一套产物作为 Debian 10 / UOS 20 x64 发布候选。

...
- Linux job 使用 `node:18-buster` / Debian 10 容器，apt 指向 archive 源，并在 `npm ci` 后强制源码重建 better-sqlite3；electron-builder 配置 `npmRebuild: false`，避免打包阶段重新安装 glibc 2.29 的预编译 native 包。CI 会在源码重建后和打包后分别检查 `better_sqlite3.node` 的最高 GLIBC 符号，超过 `GLIBC_2.28` 直接失败。CI 冒烟通过 `xvfb` + `xauth` 提供虚拟显示，并仅因 root 容器启动 Electron 时追加 `--no-sandbox`。产物输出 `Pantry-<version>-linux-amd64.deb` 与 `Pantry-<version>-linux-x86_64.AppImage`，同一套产物作为 Debian 10 / UOS 20 x64 发布候选；`.deb` 的 Maintainer 字段固定为 `Pantry Maintainers <pantry-maintainers@example.invalid>`。

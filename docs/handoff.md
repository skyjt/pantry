...
> 最后更新：2026-06-12（v0.5.2 群聊图片阈值修订候选）。**本文描述"当前状态与下一步"，会过期——以 `git log` 与各文档变更记录为准。**
...
1. **本地五连验证**：本轮群聊图片 10MB 阈值修订已于 2026-06-12 跑通 `npm test` → `npm run test:db` → `npm run typecheck` → `npm run build` → `PANTRY_UDP_PORT=47878 PANTRY_TCP_PORT=47879 npm run smoke`；后续改动仍需按同链路重跑，任何失败先修复再交付。

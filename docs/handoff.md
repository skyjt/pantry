...
> 最后更新：2026-06-12（v0.5.20，决议 #55：Linux 与 Win7 同策略默认禁硬件加速（消 UOS GPU 报错）；emoji 面板/输入框/消息三处尺寸统一（面板 16px 字号、镜像图标固定 1.3em 不随缺字槽宽缩小）；删聊天头部 no-drag 残留修复 Win7/mac 聊天区顶部不可拖。仍遵守决议 #53，每个增量 commit 必须 patch +1）。**本文描述"当前状态与下一步"，会过期——以 `git log` 与各文档变更记录为准。**
...
| GitHub 发布 | private 仓库：[skyjt/pantry_codex](https://github.com/skyjt/pantry_codex)；`.github/workflows/release.yml` 已配置 Windows 7 x64 与 Debian 10 / UOS 20 x64 构建；Linux CI 在 Debian 10 容器内强制源码重建 better-sqlite3，electron-builder 关闭二次 `npmRebuild`，并检查源码重建产物与最终包内 `.node` 最高 GLIBC 符号不超过 2.28；push `main` / 手动触发上传 artifact，推送 `v*` tag 自动创建/更新 GitHub Release；目标平台真实桌面冒烟仍按 packaging-test 执行 |
...
**一个增量 = 设计文档同步 → 实现 → `package.json` 版本号 patch +1（决议 #53）→ 五连验证 → 一个中文 commit**：

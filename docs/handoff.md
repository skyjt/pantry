...
| UI | 三栏主窗、三级通讯录树（公司▸部门▸团队）、资料卡+本地备注、全局搜索（FTS 按字）、自绘线性 SVG 系统图标、历史滚动加载、设置独立小窗、首启向导、托盘+通知直达会话 |
...
1. **本地五连验证**：本轮图标/标题/搜索框/群管理增量已于 2026-06-11 跑通 `npm test` → `npm run test:db` → `npm run typecheck` → `npm run build` → `npm run smoke`；后续改动仍需按同链路重跑，任何失败先修复再交付。

...
2. 本文 —— 状态、工作流、下一步；
3. 设计四件套（按需细读）：[requirements.md](requirements.md)（功能与 27 项决议）→ [protocol.md](protocol.md)（线上协议 v0.15）→ [ui-design.md](ui-design.md)（界面）→ [tech-design.md](tech-design.md)（选型/分层/库表）；
...
| UI | 三栏主窗、三级通讯录树（公司▸部门▸团队）、资料卡+本地备注、全局搜索（FTS 按字）、自绘线性 SVG 系统图标、输入区图标中文延迟提示、消息内容级右键菜单与边缘避让、历史滚动加载、设置独立小窗、首启向导、托盘+通知直达会话 |
...
- 决策落档：新决议追加到 requirements §9（编号已到 #27，续 #28+）+ 涉及文档的变更记录；协议改动必须 protocol.md 先行。
...
1. **本地五连验证**：最近 UI/群管理增量已于 2026-06-11 跑通 `npm test` → `npm run test:db` → `npm run typecheck` → `npm run build` → `npm run smoke`；后续改动仍需按同链路重跑，任何失败先修复再交付。
